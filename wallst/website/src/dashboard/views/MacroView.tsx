...
export function MacroView({ quotes }: Props) {
  ...
  useEffect(() => { fetch('/api/news').then(r=>r.json()).then(d=>setNews(d.slice(0,8))).catch(()=>{}); }, []);
  ...
  return (
    <div className="dc-scroll-area">
      <div className="dc-two-col">
          ...
          <div className="dc-table">
            {INDICATORS.map((ind,i) => (
              <div key={i} className="dc-table-row">
                ...
                <span className="dc-table-val" style={{color:ind.c}}>{ind.v}</span>
        ...
        <div>
          <div className="dc-section-label">MARKET PULSE</div>
          {(['SPY','QQQ','VIX'] as const).map(sym => {
            ...
            return (
              <div key={sym} className="dc-quote-row">
                ...
                <span className="dc-quote-price" style={{color: up ? '#00e676' : '#ff3b3b'}}>{q ? `$${q.c.toFixed(2)}` : '—'}</span>
                <span className="dc-quote-chg" style={{color: up ? '#00e676' : '#ff3b3b'}}>{q ? `${up?'+':''}${q.dp.toFixed(2)}%` : ''}</span>
              </div>
            );
          })}
          <div className="dc-section-label" style={{marginTop:20}}>MARKET NEWS</div>
          {news.map((item,i) => (
